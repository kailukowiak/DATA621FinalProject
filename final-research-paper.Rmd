...
Building regression models to predict house prices is not a new undertaking. Quite the opposite, a lot of research has gone into this area. There is a clear financial benefit to buyers, sellers and other parties in knowing which attributes influence the final sale price. There is also a lot of data readily available with some cleanup work. Data is kept by local governments to be used in the assessment process for property taxes. There is a lot of data captured by realtors when a property is listed on the market. Additionally, in large part thanks to the information revolution, data are easily accessible via many multiple listing services such as MLS.com.
