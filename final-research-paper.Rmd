...
There are many attributes that factor into a house price. For example, environmental attributes can impact the price substantially. Gardens, pleasant views, and attractive landscaping can all increase the house value [@luttik]. Neighborhood attributes such as schools and public services also play a factor. 
...
One key characteristic that should be taken into account is the correlation between the prices of neighboring houses [@dubin]. This will not be addressed by our project because of complexity and since we do not have specific locations for our data set. 
...
The aforementioned examples are merely scratching the surface of this particular topic. In addition to determining factors that go into house sale prices, our goal is to apply a methodical approach to linear regression. As such we have decided to study the data set without referencing other Ames housing data studies. We intentionally avoided discussions on Kaggle, the source of our data set, so that we can treat this research as a learning exercise as much as it is a scientific inquiry.
